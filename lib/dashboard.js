...
const {
  ...
  isDarkMode, getExtraUsageBg, getBarConfig, verticalBarCell, timeBarCell,
} = require('./statusline');

// Time window durations (duplicated from statusline.js which doesn't export them)
 ...
 */
function renderDashboardLines({ tier, profile, profileColor, fiveHour, weekly, extraUsage, stale, fiveHourTimePercent, weeklyTimePercent }) {
  ...
  if (config.orientation === 'vertical') {
    ...
    for (let row = 0; row < 3; row++) {
      ...
      for (let i = 0; i < percents.length; i++) {
        ...
        // Time bar immediately after each usage bar, no space separator
        if (showTime) {
          barStr += timeBarCell(
            i === 0 ? fiveHourTimePercent : weeklyTimePercent,
            i === 0 ? fiveHour.percent : (showExtraUsage ? extraUsage.percent : weekly.percent),
            row, 3
          );
        }
      }
