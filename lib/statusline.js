...
function getBarConfig() {
  ...
  try {
    ...
    if (Array.isArray(config.lines) && config.lines.length === 3 &&
        config.lines.every(l => Array.isArray(l) && l.every(e => typeof e === 'string'))) {
      lineElements = config.lines;
    }
  } catch {}
  ...
  _barConfig = { width, emptyBg, timeBarBg, gradient, truecolor, isRgb, partialBlocks: usePartialBlocks, orientation, cwdMaxLength, cwdDepth, showTimeBars, lineElements };
  return _barConfig;
}

const OAUTH_CLIENT_ID = '9d1c250a-e61b-44d9-88ed-5944d1962f5e';

// Cache TTL: 60s for success, 60s base for failures (backoff caps at 5min)
const CACHE_TTL_MS = 60_000;
const ERROR_CACHE_TTL_MS = 60_000;
const MAX_ERROR_CACHE_TTL_MS = 300_000;
...
/**
 * Run an async function with a timeout. Rejects with Error('timeout') if
 * the function doesn't settle within `ms` milliseconds.
 */
...
async function getUsageData(claudeDir, forceRefresh = false) {
  ...
  if (!forceRefresh && cache && cache.timestamp) {
    ...
    const shouldBypassTtl = resetPassed;
  ...
  // Combined 7s timeout: a slow token refresh still leaves time for the API
  // call, and total render time is bounded even if both endpoints are sluggish.
  let accessToken, apiData;
  try {
    [accessToken, apiData] = await withTimeout(7000, async () => {
      const token = await getValidToken(claudeDir);
  ...
  if (apiData && apiData.error === 'auth') {
    try {
      writeFileAtomic(cacheFile, JSON.stringify({
        ...
        fiveHour: { percent: cache?.fiveHour?.percent ?? null, resetAt: cache?.fiveHour?.resetAt ?? null },
        weekly: { percent: cache?.weekly?.percent ?? null, resetAt: cache?.weekly?.resetAt ?? null },
...
function buildLineText(elementMap, elementList) {
  if (!elementList) return '';
  return elementList
    .map(name => { const fn = elementMap[name]; return fn ? fn() : null; })
    .filter(v => v != null && v !== '')
    .join(' ');
}

function shortenModelName(model) {
  const m = /^claude-(opus|sonnet|haiku)-(\d+)-(\d+)(?:-(\d{1,2}))?(?:-\d{8})?$/.exec(model);
  if (!m) return model;
  const initial = m[1][0].toUpperCase();
  const patch = m[4] && m[4] !== '0' ? '.' + m[4] : '';
 ...
 */
function renderLines({ context, model, tier, elapsed, profile, profileColor, ghUser, fiveHour, weekly, extraUsage, stale, lastSuccessTs, git, lines: lineChanges, cwd, fiveHourTimePercent, weeklyTimePercent, ccVersion }) {
  const config = getBarConfig();
  if (!config.lineElements) {
    return '\n\n';
  }
  const lines = [];
  ...
  // Colored via hash hue of the bare username, matching the profile name strategy.
  const ghUserStr = ghUser ? `${hueToAnsi(hashToHue(ghUser), isDarkMode())}@${ghUser}${colors.reset}` : '';
  const ccVersionStr = ccVersion ? `${colors.dim}${ccVersion}${colors.reset}` : '';
  ...
  const gitStr = git.hasGit
    ? `${colors.cyan}${git.branch}${colors.reset}` +
      (git.ahead > 0 ? ` ${colors.magenta}\u2191${git.ahead}${colors.reset}` : '') +
      (git.behind > 0 ? ` ${colors.magenta}\u2193${git.behind}${colors.reset}` : '') +
  ...
  if (config.orientation === 'vertical') {
    ...
    for (let row = 0; row < 3; row++) {
      ...
      if (row === 0) {
        const text = buildLineText({
          context: () => `${colors.cyan}${Math.round(context)}%${colors.reset}`,
          elapsed: () => elapsedStr,
          profile: () => profileStr,
          tier: () => `${colors.magenta}${tier}${colors.reset}`,
          model: () => `${colors.white}${shortenModelName(model)}${colors.reset}`,
          version: () => ccVersionStr,
        }, config.lineElements[0]);
        lines.push(`${barStr}${text ? ' ' + text : ''}`);
      } else if (row === 1) {
        const text = buildLineText({
          usage5h: () => fiveHourPercent,
          staleness: () => ageSuffix,
          age: () => `${colors.dim}${fiveHourReset}${colors.reset}`,
          ghUser: () => ghUserStr,
          branch: () => gitStr + (linesStr ? ` ${linesStr}` : ''),
        }, config.lineElements[1]);
        lines.push(`${barStr}${text ? ' ' + text : ''}`);
      } else {
        const thirdPct = showExtraUsage
          ...
          : weeklyPercent;
        const text = buildLineText({
          usageWeekly: () => thirdPct,
          staleness: () => ageSuffix,
          age: () => `${colors.dim}${weeklyReset}${colors.reset}`,
          cwd: () => `${colors.white}${cwd}${colors.reset}`,
        }, config.lineElements[2]);
        lines.push(`${barStr}${text ? ' ' + text : ''}`);
      }
    }
  } else {
    ...
    lines.push(`${contextBar}${text1 ? ' ' + text1 : ''}`);
    ...
    lines.push(`${fiveHourBar}${text2 ? ' ' + text2 : ''}`);
    ...
    lines.push(`${thirdBar}${text3 ? ' ' + text3 : ''}`);
  }
...
async function main() {
  // Ensure lineElements is set so renderLines produces output even without config
  const cfg = getBarConfig();
  if (!cfg.lineElements) {
    cfg.lineElements = [
  ...
  const model = (typeof stdinData.model === 'string' ? stdinData.model : null)
    || stdinData.model?.display_name || stdinData.model?.id || '?';
  const ccVersion = (() => {
    const execpath = process.env.CLAUDE_CODE_EXECPATH;
    if (execpath) return 'v' + execpath.split('/').pop().replace(/-/g, '.');
    const agent = process.env.AI_AGENT;
    if (!agent) return null;
    const m = agent.match(/^claude-code_([\d_-]+?)_/);
  ...
  // Prefer rate_limits from stdin (Claude Code provides this directly,
  // no OAuth API call needed). Fall back to OAuth API when unavailable.
  const stdinRateLimits = stdinData.rate_limits;
  const hasStdinUsage = stdinRateLimits?.five_hour?.used_percentage != null;

  let usagePromise;
  if (hasStdinUsage) {
    const now = Date.now();
    const fiveHourResetAt = stdinRateLimits.five_hour.resets_at
      ? stdinRateLimits.five_hour.resets_at * 1000 : null;
    const weeklyResetAt = stdinRateLimits.seven_day?.resets_at
      ? stdinRateLimits.seven_day.resets_at * 1000 : null;
    const stdinExtra = stdinRateLimits.extra_usage;
    // Write stdin rate_limits to cache so other sessions get fresh data
    const cacheFile = path.join(claudeDir, '.statusline-cache.json');
    try {
      writeFileAtomic(cacheFile, JSON.stringify({
        timestamp: now,
        ...
        status: 'ok',
        consecutiveErrors: 0,
        fiveHour: { percent: stdinRateLimits.five_hour.used_percentage, resetAt: fiveHourResetAt },
        weekly: { percent: stdinRateLimits.seven_day?.used_percentage ?? null, resetAt: weeklyResetAt },
        extraUsage: { percent: stdinExtra?.utilization ?? null, enabled: !!stdinExtra?.is_enabled },
      }));
    } catch {}
    usagePromise = Promise.resolve({
      ...
      lastSuccessTs: now,
      fiveHour: {
        percent: stdinRateLimits.five_hour.used_percentage,
        resetIn: fiveHourResetAt ? Math.max(0, fiveHourResetAt - now) : null,
      },
      weekly: {
        percent: stdinRateLimits.seven_day?.used_percentage ?? null,
        resetIn: weeklyResetAt ? Math.max(0, weeklyResetAt - now) : null,
      },
      extraUsage: {
        percent: stdinExtra?.utilization ?? null,
        enabled: !!stdinExtra?.is_enabled,
      },
    });
  } else {
    ...
    usagePromise = isOAuth
      ? getUsageData(claudeDir, isSessionStart || credentialsChanged)
      : Promise.resolve({ stale: false, fiveHour: { percent: null, resetIn: null }, weekly: { percent: null, resetIn: null }, extraUsage: { percent: null, enabled: false } });
  }
 ...
 *
 * Merges up to three sources, deduped by resolved path:
 *   1. "profiles" array from ~/.config/howmuchleft.json (explicit paths)
 *   2. claudewheel's options.json profile metadata (unless "excludeClaudewheel" is set)
 *   3. Scanning ~/.claude and ~/.claude-* dirs with .credentials.json
 *
 * Returns an array of absolute directory paths, sorted by creation time.
 */
function discoverProfiles() {
  ...
  try {
    const raw = fs.readFileSync(CONFIG_PATH, 'utf8');
    const config = parseJsonc(raw);
    if (config.excludeClaudewheel) excludeCw = true;
    if (Array.isArray(config.profiles)) {
      for (const p of config.profiles) {
        try { if (fs.statSync(p).isDirectory()) found.add(p); } catch {}
      }
    }
  } catch {}

  // claudewheel's config (~/.claudewheel/ or legacy ~/.claudelauncher/)
  if (!excludeCw) {
    for (const cwDir of ['.claudewheel', '.claudelauncher']) {
      try {
        const optRaw = fs.readFileSync(path.join(homeDir, cwDir, 'options.json'), 'utf8');
        const opts = JSON.parse(optRaw);
        const meta = opts && opts.profile && opts.profile.metadata;
        if (meta) {
          for (const key of Object.keys(meta)) {
            const dir = meta[key] && meta[key].config_dir;
            if (!dir) continue;
            const resolved = dir.replace(/^~/, homeDir);
            try { if (fs.statSync(resolved).isDirectory()) found.add(resolved); } catch {}
          }
        }
        break;
      } catch {}
  ...
  // Scan home directory for ~/.claude and ~/.claude-*
