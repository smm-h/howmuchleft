...
// Time bar background: halfway between bar bg and terminal default (more subtle).
// Dark terminals default to black (0,0,0), light to white (255,255,255).
function computeTimeBarBg(rawBg, dark, truecolor) {
  const termDefault = dark ? 0 : 255;
  if (isRgbStop(rawBg)) {
    const mid = [
      Math.round((rawBg[0] + termDefault) / 2),
      Math.round((rawBg[1] + termDefault) / 2),
      Math.round((rawBg[2] + termDefault) / 2),
    ];
  ...
  // Shift index halfway toward terminal default end of the grayscale ramp.
  const base = Number.isInteger(rawBg) ? rawBg : (dark ? 236 : 252);
  const target = dark ? 232 : 255;
  const mid = Math.round((base + target) / 2);
...
function getBarConfig() {
  ...
  // Time bar bg: halfway between the bar bg and the terminal default (dimmer)
  const rawBg = userMatch?.bg ?? builtinMatch?.bg;
  const timeBarBg = computeTimeBarBg(rawBg, dark, truecolor);
...
function renderLines({ context, model, tier, elapsed, profile, profileColor, fiveHour, weekly, extraUsage, stale, git, lines: lineChanges, cwd, fiveHourTimePercent, weeklyTimePercent }) {
  ...
  if (config.orientation === 'vertical') {
    ...
    // 3rd bar: extra usage replaces weekly when weekly >= 100% and extra is enabled
    const thirdPercent = showExtraUsage ? extraUsage.percent : weekly.percent;
