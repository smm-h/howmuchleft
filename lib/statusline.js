...
function getBarConfig() {
  ...
  const fractionalChars = orientation === 'vertical' ? VERTICAL_CHARS : HORIZONTAL_CHARS;
  _barConfig = { width, emptyBg, gradient, truecolor, isRgb, partialBlocks: usePartialBlocks, fractionalChars, orientation };
  return _barConfig;
}
...
function progressBar(percent, width) {
  ...
  for (let i = 0; i < width; i++) {
    ...
    if (cellEnd <= fillFrac) {
      ...
      const chars = config.fractionalChars;
 ...
 */
function renderLines({ context, model, tier, elapsed, profile, fiveHour, weekly, stale, git, lines: lineChanges, cwd }) {
  const config = getBarConfig();
  const lines = [];

  // Pre-compute labels shared by both modes
  const elapsedStr = elapsed != null ? `${colors.dim}${formatTimeRemaining(elapsed)}${colors.reset} ` : '';
  const profileStr = profile ? `${colors.dim}${profile}${colors.reset} ` : '';
