...
// Default gradients: green -> yellow -> orange -> red (256-color indices)
// Dark mode: bright colors (level 5) for dark backgrounds
const DEFAULT_GRADIENT_DARK = [46, 82, 118, 154, 190, 226, 220, 214, 208, 202, 196];
// Light mode: darker colors (level 3-4) visible against light backgrounds
const DEFAULT_GRADIENT_LIGHT = [28, 64, 100, 136, 172, 178, 172, 166, 160, 124, 88];
...
// Resolve gradient from config: gradientDark/gradientLight take priority,
// then "gradient" (applies to both modes), then built-in defaults.
function resolveGradient(config, dark) {
  const modeKey = dark ? 'gradientDark' : 'gradientLight';
  if (isValidGradient(config[modeKey])) return config[modeKey];
  if (isValidGradient(config.gradient)) return config.gradient;
  return dark ? DEFAULT_GRADIENT_DARK : DEFAULT_GRADIENT_LIGHT;
}
...
function getBarConfig() {
  ...
  try {
    ...
    _barConfig = {
      ...
      gradient: resolveGradient(config, dark),
    };
  } catch {
    const dark = isDarkMode();
    _barConfig = {
      width: 12,
      emptyBg: dark ? 236 : 252,
      gradient: dark ? DEFAULT_GRADIENT_DARK : DEFAULT_GRADIENT_LIGHT,
    };
  }
  return _barConfig;
}
...
module.exports = { main, CONFIG_PATH, progressBar, formatPercent, formatTimeRemaining, colors };
