...
// Braille: left-column pins only, filling bottom-to-top within each character
// Dot positions (top to bottom): 1 (0x01), 2 (0x02), 3 (0x04), 7 (0x40)
const BRAILLE_EMPTY = '\u2800';         // empty braille pattern (blank space)
const BRAILLE_FULL_LEFT = '\u2847';     // all 4 left pins lit (the "track" pattern)
...
function getBarConfig() {
  ...
  try {
    ...
    if (config.progressBarStyle === 'block' || config.progressBarStyle === 'braille') barStyle = config.progressBarStyle;
  } catch {}
  ...
  _barConfig = { width, emptyBg, timeBarBg, gradient, truecolor, isRgb, partialBlocks: usePartialBlocks, orientation, cwdMaxLength, cwdDepth, showTimeBars, barStyle };
  return _barConfig;
}
...
async function getUsageData(claudeDir, forceRefresh = false) {
  ...
  if (apiData && apiData.error === 'auth') {
    ...
    return { stale: false, fiveHour: { percent: null, resetIn: null }, weekly: { percent: null, resetIn: null }, extraUsage: { percent: null, enabled: false } };
  }
...
/**
 * Render one cell of a braille vertical bar.
 * Uses left-column braille dots only: 4 levels per char, fills bottom-to-top.
 * Unfilled portion shows faint full-column dots as a visible "track".
 * No background color -- bare terminal background.
 */
function brailleBarCell(percent, rowIdx, totalRows) {
  if (totalRows == null) totalRows = 3;
  const dark = isDarkMode();
  const fg = dark ? '\x1b[37m' : '\x1b[30m';       // white or black
  const dimFg = '\x1b[2m' + fg;                      // dim version for unfilled track
  const clamped = (percent == null) ? 0 : Math.max(0, Math.min(100, percent));
  const maxLevel = totalRows * 4;  // 4 levels per row, not 8
  const level = Math.round(clamped / 100 * maxLevel);

  // Each row covers 4 levels; bottom row fills first
  const rowBase = (totalRows - 1 - rowIdx) * 4;
  const rowLevel = level - rowBase;

  if (rowLevel >= 4) {
    // Fully filled cell
    return fg + BRAILLE_FULL_LEFT + colors.reset;
  } else if (rowLevel > 0) {
    // Partially filled -- show the filled braille char
    return fg + BRAILLE_LEVELS[rowLevel - 1] + colors.reset;
  } else {
    // Unfilled -- show faint track dots
    return dimFg + BRAILLE_FULL_LEFT + colors.reset;
  }
}

/**
 * Braille variant of time bar cell.
 * Same fill logic as brailleBarCell but monochrome (no urgency coloring).
 */
function brailleTimeBarCell(timePercent, rowIdx, totalRows) {
  if (totalRows == null) totalRows = 3;
  const dark = isDarkMode();
  const fg = dark ? '\x1b[37m' : '\x1b[30m';
  const dimFg = '\x1b[2m' + fg;
  const clamped = (timePercent == null) ? 0 : Math.max(0, Math.min(100, timePercent));
  const maxLevel = totalRows * 4;
  const level = Math.round(clamped / 100 * maxLevel);
 ...
 */
function renderLines({ context, model, tier, elapsed, profile, profileColor, fiveHour, weekly, extraUsage, stale, git, lines: lineChanges, cwd, fiveHourTimePercent, weeklyTimePercent }) {
  const config = getBarConfig();
  const lines = [];
  ...
  if (config.orientation === 'vertical') {
    ...
    for (let row = 0; row < 3; row++) {
      ...
      for (let i = 0; i < percents.length; i++) {
        ...
        // Use warm bg for 3rd bar when showing extra usage (block style only)
        const bgOverride = (i === 2 && warmBg) ? warmBg : null;
        if (config.barStyle === 'braille') {
          barStr += brailleBarCell(percents[i], row, 3);
        } else {
...
function testColors() {
  ...
  console.log(`Mode: ${dark ? 'dark' : 'light'}, Color: ${tc ? 'truecolor' : '256-color'}, Width: ${config.width}, Partial blocks: ${config.partialBlocks ? 'on' : 'off'}, Orientation: ${orient}, Style: ${config.barStyle}`);
  console.log();
  ...
  for (let i = 0; i < pcts.length; i++) {
    ...
    line += '  ';
    for (const vp of pcts) {
      line += brailleBarCell(vp, i, pcts.length);
    }
    line += colors.reset;
    console.log(line);
  }
...
module.exports = {
  ...
  timeBarCell, getUrgencyColor, brailleBarCell, brailleTimeBarCell,
};
