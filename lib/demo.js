...
const { renderLines, getBarConfig } = require('./statusline');
...
function renderFrame(state, isFirst) {
  ...
  const output = renderLines({
    ...
    fiveHourTimePercent: state.fiveHourTimePercent,
    weeklyTimePercent: state.weeklyTimePercent,
  });
...
function runDemo(durationSec = 60) {
  ...
  const interval = setInterval(() => {
    ...
    // Time advances linearly over full duration while usage front-loads to 70%,
    // so the time bar naturally shows urgency (usage outpaces time).
    const weeklyTimePercent = isLast ? 100 : t * 100;

    // 5-hour: 8 sawtooth cycles (last frame pinned to 100%)
    const fiveCycleT = (t * FIVE_HOUR_CYCLES) % 1;
    const fiveHour = isLast ? 100 : Math.pow(fiveCycleT, 0.4) * 100;
    const fiveHourResetIn = isLast ? 0 : (1 - fiveCycleT) * FIVE_HOUR_MS;
    // Usage ramps via power curve (front-loaded), time is linear within each cycle,
    // creating visible urgency that builds then resets each cycle.
    const fiveHourTimePercent = isLast ? 100 : fiveCycleT * 100;
