...
function runDemo(durationSec = 60) {
  ...
  const interval = setInterval(() => {
    ...
    const weekly = t * 100;
    const weeklyResetIn = (1 - t) * SEVEN_DAY_MS;

    // 5-hour: 8 sawtooth cycles
    const fiveCycleT = (t * FIVE_HOUR_CYCLES) % 1;
    const fiveHour = fiveCycleT * 100;
    const fiveHourResetIn = (1 - fiveCycleT) * FIVE_HOUR_MS;

    // Context: 15 sawtooth cycles
    const ctxCycleT = (t * CONTEXT_CYCLES) % 1;
    const context = ctxCycleT * 100;
