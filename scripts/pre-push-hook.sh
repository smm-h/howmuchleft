...
if [ -f .share-it-on/version ]; then
  SCAFFOLD_VER=$(cat .share-it-on/version | tr -d '[:space:]')
  if command -v share-it-on &>/dev/null; then
    CURRENT_VER=$(share-it-on --version 2>/dev/null | tr -d '[:space:]')
    if [ -n "$CURRENT_VER" ] && [ "$SCAFFOLD_VER" != "$CURRENT_VER" ]; then
      echo "Warning: scaffolding was generated by share-it-on $SCAFFOLD_VER but you have $CURRENT_VER installed."
      echo "Run 'share-it-on scaffold --update' to update scaffolding."
    fi
  fi
fi
