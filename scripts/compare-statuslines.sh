...
#
# The results go to .stricttools/docs/statusline-comparison.toml, which the
# selfdoc directive `statusline-comparison` renders into
# .stricttools/docs/comparison.md.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RESULTS_FILE="$REPO_ROOT/.stricttools/docs/statusline-comparison.toml"
...

{
  ...
  echo "# Rendered into .stricttools/docs/comparison.md by the statusline-comparison directive."
  echo
  echo "[run]"
